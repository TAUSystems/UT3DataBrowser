...
def plot_pointing_and_spectrum_plotly(burst_timestamp: datetime, shot_timestamp: datetime):
    config = load_config()
    e_spectrometer_folder = Path(config['directories']['epics_daq_test_folder_path']) / 'data' / f"burst-{burst_timestamp:%Y-%m-%dT%H-%M-%S-%fZ}" / f"shot-{shot_timestamp:%Y-%m-%dT%H-%M-%S-%fZ}" / 'E-Spectrometer'
    ...
    if burst_timestamp < datetime(2024, 11, 2, 1, 6, 32):
        low_energy_image, high_energy_image = preprocess_spectrum_images(low_energy_image, high_energy_image)

    low_energy_image_nonoverlap_region, overlap_image, high_energy_image_nonoverlap_region = stitch_spectrum_images(low_energy_image, high_energy_image)
    stride = 3

    fig = make_subplots(rows=1, cols=2, subplot_titles=("Pointing Image", "Spectrum Image"),
                        column_widths=[0.3, 0.7],
                        specs=[[{"type": "heatmap"}, {"type": "heatmap"}]])
    fig.add_trace(
        go.Heatmap(
            z=pointing_image.image[::stride, ::stride],
            x=pointing_image.x_axis[::stride],
            y=pointing_image.y_axis[::stride],
            colorscale='Viridis',
            zmin=0,
            zmax=config['plot']['pointing_intensity_max'],
            colorbar=dict(title='Intensity', x=0.25)
        ),
        row=1, col=1
    )
    def add_spectrum_heatmap(image: ImageWithAxes):
        if image.image.size:
            fig.add_trace(
                go.Heatmap(
                    z=image.image[::stride, ::stride],
                    x=image.x_axis[::stride],
                    y=image.y_axis[::stride],
                    colorscale='Viridis',
                    zmin=0,
                    zmax=config['plot']['spectrum_intensity_max'],
                    showscale=False
                ),
                row=1, col=2
            )
    add_spectrum_heatmap(low_energy_image)
    add_spectrum_heatmap(high_energy_image_nonoverlap_region)
    fig.add_trace(
        go.Scatter(
            x=spectrum_lineout_energy_axis,
            y=spectrum_lineout,
            mode='lines',
    ...
    fig.update_xaxes(title_text="horizontal angle [mrad]", row=1, col=1)
    fig.update_yaxes(title_text="vertical angle [mrad]", row=1, col=1)

    fig.update_xaxes(title_text="energy [MeV]", row=1, col=2)
    fig.update_yaxes(title_text="horizontal angle [mrad]", row=1, col=2)

    fig.update_layout(
        title="Pointing and Spectrum Images",
        height=500,
        width=1200,
        yaxis2=dict(overlaying='y', side='right', showticklabels=False, range=[0, config['plot']['spectrum_lineout_max']])
    )
